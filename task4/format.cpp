...
std::string read_format(const std::string &formatstr, size_t &index, bool &width_arg, bool &precision_arg, Format &f)
{
	...
	for (index; index < len; index++)
	{
		...
		return reeadFA(formatstr, index, width_arg, precision_arg, f);
	}
	return formatstr.substr(start, index - start);
}
...
std::string reeadFA(const std::string &fmt, size_t &index, bool &width_arg, bool &precision_arg, Format &format)
{
